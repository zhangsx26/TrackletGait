...
class TripletLoss_HardEx(BaseLoss):
    def __init__(self, margin, loss_term_weight=1.0):
        super(TripletLoss_HardEx, self).__init__(loss_term_weight)
        self.margin = margin
    ...
    def forward(self, embeddings, labels):
        ...
        ###
        p, n, n2, n3 = ap_dist.shape
        _, _, _, n4 = an_dist.shape
        mean_dist =  ap_dist.mean((1,2))
        max_dist, _ = torch.max(ap_dist.view(dist.size(0), -1), dim=-1)
        max_dist = max_dist.unsqueeze(-1)
    
        mask = (max_dist - mean_dist) *2/3 + mean_dist
        mask = mask.unsqueeze(-1).unsqueeze(-1).repeat(1, n, n2, n3)
        mask = torch.ge(ap_dist,mask)
        mask = mask.repeat(1, 1, 1, n4).view(dist.size(0), -1)

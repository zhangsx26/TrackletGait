...
class CollateFn(object):
    def __init__(self, label_set, sample_config):
        ...
        self.method = sample_config['method']
        
        self.sample_tracklet_length = sample_config['sample_tracklet_length']
        self.sample_tracklet_prob = sample_config['sample_tracklet_prob']
        sample_tracklet_step = sample_config['sample_tracklet_step']
        
        self.tracklet_n = 0
        self.tracklet_l = int(self.sample_tracklet_length[0])
        self.tracklet_s = int(sample_tracklet_step)
    ...
    def __call__(self, batch):
        ...
        def sample_frames(seqs):
            ...
            ###                
            elif self.method == 'tracklet':
                indices = []
                id_index_list = list(range(seq_len))

                for i in range(self.tracklet_n):
                    it = 0
                    fs_n = (self.tracklet_l*self.tracklet_s-self.tracklet_s+1)
                    if seq_len < fs_n:
                        it = math.ceil(fs_n / seq_len)+1
                        seq_len = seq_len * it
                        id_index_list = id_index_list * it
  
                    start = random.choice(list(  range(0, seq_len - fs_n + 1)  ))
              
                    for j in range(self.tracklet_l):
                        indices.append( id_index_list[start+(j*self.tracklet_s)])
                        
                        
            elif self.method == 'tracklet_mix_p':
                indices = []
                id_index_list = list(range(seq_len))
                
                # tracklet_l = random.choice([8,16,32])
                tracklet_l = int(random.choices(self.sample_tracklet_length, weights=self.sample_tracklet_prob, k=1)[0])
                tracklet_n = 32//tracklet_l # %%%%% 32 

                for i in range(tracklet_n):
                    it = 0
                    fs_n = (tracklet_l*self.tracklet_s-self.tracklet_s+1)
                    if seq_len < fs_n:
                        it = math.ceil(fs_n / seq_len)+1
                        seq_len = seq_len * it
                        id_index_list = id_index_list * it
